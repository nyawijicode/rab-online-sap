...
class EditQcTask extends EditRecord
{
    ...
    {
        // Merge both strict and manual items to process them identically
        $allItemData = array_merge(
            $data['items_strict'] ?? [],
            $data['items_manual'] ?? []
        );

        foreach ($allItemData as $itemData) {
            $task = QcTask::find($itemData['id']);
            if (!$task) continue;

            $task->update([
                'serial_number' => $itemData['serial_number'] ?? $task->serial_number,
                'scanned_serial_number' => $itemData['scanned_serial_number'],
                'qty_pass' => $itemData['qty_pass'],
                'qty_fail' => $itemData['qty_fail'],
            ...
            // Save checklist for this specific task
            if (isset($itemData['checklist'])) {
                foreach ($itemData['checklist'] as $criteriaId => $isChecked) {
                    QcTaskCriteria::updateOrCreate(
                        [
                            'qc_task_id' => $task->id,
                            'qc_criteria_id' => $criteriaId,
                        ],
                        [
